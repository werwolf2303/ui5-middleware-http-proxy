...
const _resolveUri = ({ path, req }) => {
  ...
  // Fixes // when not specifying a path
  if (resolvedUri.startsWith("//")) resolvedUri = resolvedUri.slice(1)
  return resolvedUri
}
...
module.exports = ({ options: { configuration = {} } }) => {
  ...
  return async (req, res, next) => {
    ...
    const options = {
      ...
      redirect: 'manual',
      // If secure is false, it selects the agent based on the baseUrl protocol e.g. https://www.google.de -> https.Agent, http://www.google.de -> undefined
      agent: secure ? undefined : baseUrl.startsWith("http:") ? undefined : new https.Agent({ rejectUnauthorized: false })
    }

    if (options.headers.cookie) {
      await jar.setCookie(options.headers.cookie, baseUrl)
    ...
    if (resolvedAuth) {
      options.headers.Authorization = `Basic ${resolvedAuth}`
    }

    // actual request
    fetchWithCookies(baseUrl + resolvedUri, options).then(response => {
      // forward headers
